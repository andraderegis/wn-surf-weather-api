...
export class Rating {
  ...
  public getRateForPoint(point: ForecastPoint): number {
    const swellDirection = this.getPositonFromLocation(point.swellDirection);

    const windDirection = this.getPositonFromLocation(point.windDirection);
  ...
  public getPositonFromLocation(coordinates: number): GeoPosition {
